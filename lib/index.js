...
function acceptance(Gj, Gi, r, T, source) {
  var pi = pijOverpii(Gj, Gi, r, T, source);
  var pji = proposalDistribution(Gj, Gi);
  var pij = proposalDistribution(Gi, Gj);
  var P = Math.min(1, (pi * pji) / pij);
  return P;
}
...
function markovChain(userInput) {
  ...
  for (var j = 0; j < runtime; j++) {
    ...
    var accept = acceptance(Gj, Gi, r, T, sourceNode);
    // Console.log(accept);
    if (accept >= randomGeneratedProbability) {
      numberOfEdges.push(Gj.edges().length);
      numberOfEdgesToSource.push(edgeConnectedToSource(Gj, sourceNode));
      maximumDistance.push(maxDistance(Gj, sourceNode));
      Gi = cloneGraph(Gj);
    }
  }
