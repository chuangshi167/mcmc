...
function pijOverpii(Gj, Gi, T, r, source) {
  /*
  This function calculates the pi_j Over pi_i based on the current state and the expected state, T, r and source
  Args:
  Gj expected state: graph
  Gi current: graph
  T: number
  r: number
  source: node
...
function proposalDistribution(Gj, Gi) {
  /*
  This function calculates the proposal distribution going from the current state to the expected state.
  Args: 
  Gj: graph (expected state)
  ...
  */
  ...
  // If an edge is added to the current state to go to the expected state
  if (Gj.edges().length > Gi.edges().length) {
    return 1 / ((M * (M - 1)) / 2 - E);
  }
  // If an edge is removed from the current state to go to the expected state
  if (Gj.edges().length < Gi.edges().length) {
    return 1 / (E - B);
  }
}
