...
function logHistory(G, history) {
  /*
  This function takes the current state and state history map as input, and add counts to the appearance of the current state
  Args:
  ...
  */
  ...
  history.forEach((value, key, map) => {
    // If the current state has appeared in the past, count + 1;
    if (isEqual(key, G)) {
      map.set(key, ++value);
      flag = false;
    }
  });
  // If the current state has never been achieved, count = 1;
  if (flag) {
    history.set(G, 1);
  }
}
...
function convert(coordinates) {
  /*
  This function converts the input coordinate string to coordinate array
  Args:
  coordinates: string
  Return:
  coor: array
...
function maxDistance(G, sourceNode) {
  /*
  This function find the maxmium of the shortest distance to each node from the source.
  Args:
  G: current state (graph)
  sourceNode: node
  Returns:
  max: the maxmium of the shortest distance to each node from the source (number)
  */
  let max = 0;
...
function edgeConnectedToSource(G, sourceNode) {
  /*
  This function finds the number of edges connecting to the source node"
  Args:
  G: current state (graph)
  sourcennode: node
  Return:
  e: number of edges connecting to the source node (number)
  */
  var e = 0;
...
function expectedValue(targetList, weightList) {
  /*
  This function calculates the expected value from a list of elements and a list cotaning the weight of each element.
  Args:
  targetList: array
  weightList: array
  Return:
...
function markovChain(input) {
  /*
  This function is the main function that runs the Markov Chain simulation, based on the user input
  Args:
  input: user input
  Return:
  history: map that contains the appeared graph and their frequency
  */
  ...
  // Created a new graph based on the input
  ...
  for (var i = 0; i < iterations; i++) {
    ...
    // Proposing the next state
    if (aOr) {
      addRandomEdge(Gj);
    } else if (!aOr) {
      removeRandomEdge(Gj);
    }
    // Deciding whether the proposed state will be accepted or not
...
function main(input) {
  /*
  This function is the function that deals with the outcome of Markov Chain Simulation
  Args:
  input: user input
  Return:
  No return
  */
  ...
  var largestFrequency = Math.max.apply(null, graphDistributionList);
  var index = graphDistributionList.indexOf(largestFrequency);
  console.log('The adjacency matrix of top 1% most probable graph: is ');
  console.log(adjacencyMatrix(historyGraph[index]));
}
